...
**Each pane zooms on its own, from buttons in its window chrome.** The `−` / `+` pair beside the traffic lights steps that window through `0.5 … 3`, and the level shows in the bar whenever it is not 100%.

The controls are in the chrome because nothing else can reach these panes: mouse and keyboard events over an iframe go to the iframe's document, and code-server is on another port while an embedded site is on another domain — so neither a wheel gesture nor a keyboard shortcut on the slide can be captured. A button in our own DOM works for every pane type regardless of origin.

`demoit.js` sets `--pane-zoom` on the `<fake-window>` element on a `demoit:zoom` event. The content it frames — an iframe, or the code viewer's container — is a DOM child of that element, so the custom property inherits straight through the shadow boundary a class could never cross. Each component then applies `zoom: var(--pane-zoom, 1)` and **leaves its width and height alone**: under the `zoom` property a percentage size still resolves against the un-zoomed parent, so the box keeps its place on the slide while only the content grows. Dividing the size by the zoom as well — the obvious-looking thing — shrinks the pane instead. `zoom` rather than a transform, too: it re-lays-out at the new scale, so a terminal reflows its columns and stays crisp instead of being rasterised and stretched. The code viewer has no iframe, so it scales `--source-code-font-size` instead and needs no box arithmetic.

A zoom is per window and per page load: it is not remembered across a navigation, and a multi-tab terminal zooms each tab separately.

**Maximising a window takes the stage out of the way.** The green dot puts the window `position: fixed`, which resolves against the viewport — unless an ancestor is transformed, in which case that ancestor becomes the containing block. So on the stage a maximised window became a fixed 1920×1080 box in *stage* coordinates, with two visible consequences: a sibling terminal iframe kept painting over a maximised browser, and a browser zoom grew the content inside a box that could not grow with it, putting scrollbars inside the terminal and the embedded page.

`demoit.js` therefore does two things on a `demoit:maximize` event. It adds `.stage-maximized`, whose `transform: none` hands the window back to the viewport (with `100vw`/`100vh` so no slide content peeks from behind it) — this is what restores the pre-stage behaviour, where a maximised pane filled the screen and zoom grew its content while it kept filling the screen. And it hides the slide's other panes outright, which is deterministic where stacking was not, and is what maximising is for anyway.

It hides them with `visibility`, never `display`: `display: none` would tear a tty or code-server iframe down and bring it back empty, losing whatever the speaker had typed mid-demo.
...
- **Browser zoom still works, and the fit has to be written for it.** A page zoom of Z shrinks `innerWidth` in CSS pixels by exactly Z, so a scale computed from `innerWidth` alone cancels the zoom out — and since the transform applies to everything inside the stage, the tty and the embedded pages stop zooming too. `fitStage` therefore multiplies the fit by `devicePixelRatio / <its value at load>`, which is the only handle a page has on the zoom level. That baseline is stale if the window moves to a display of a different density; a reload fixes it.
- **The stage is anchored top-left and centred by a computed offset**, not by `translate(-50%, -50%)`. The difference only shows under zoom: a percentage translation puts half the overflow above and left of the viewport, where nothing can scroll to it.
...
**The stage carries the decor, as two pseudo-elements.** `.stage::before` is the dot grid (`radial-gradient` of `--color-main`, 32px), `.stage::after` the mesh gradient (four radial ellipses on the accents, masked out towards the bottom) — both taken from zatsit.fr's `.dot-grid` and `.hero-gradient`. Their strength is two tokens, `--decor-dots` and `--decor-mesh`, one per layer because the site lowers its mesh in dark mode and leaves its dot grid alone.

On the stage rather than on the slides, and that is the point: no slide's markup mentions it, no layout has to know about it, and it cannot shift any geometry. The site's floating blobs are deliberately **not** ported — this deck runs ttys and live iframes for three quarters of an hour, and an animated `blur(80px)` beside them is a permanent cost for an effect nobody looks at.

Both pseudo-elements sit at **`z-index: -1`**, which is what keeps them from touching anything else: a child at a negative z-index paints above its stacking context's background and below the in-flow content, so the decor slips under the slides without a single child needing to be repositioned. At `z-index: 0` the children would have needed a `.stage > * { position: relative }`, and that unlayered rule would beat the `@utility`-generated `.slide-footer` in `@layer utilities` — dropping the footer and the progress bar back into the flow. `.stage` also carries `isolation: isolate`, because `.stage-maximized` sets `transform: none` and would otherwise destroy the stacking context its transform provides.

**`cover` and `quote` push both layers**, via `slide-hero` on their `<main>` and a `.stage:has(main.slide-hero)` rule — `:has()` because a layout cannot reach its own container, which belongs to `index.tmpl.html`. That rule **must** stay guarded by `:root:not([data-display="projector"])`: a custom property resolves from the nearest element that declares it, and specificity plays no part across two elements, so an unguarded `.stage` declaration silently beats the `:root` one the projector profile sets — extinguishing the decor on working slides only, and never on a cover. Note also that a slide declaring `class:` replaces a layout's default class list wholesale, so such a slide drops `slide-hero` with nothing to signal it.
...
**Theme and display profile are a class and an attribute on `<html>`.** `.dark` swaps the palette — and it swaps the *hue*, not just the lightness: the site's primary is blue `#0f15fd` in light and gold `#f1be51` in dark, with an amber / red / orange accent family replacing the cyan / blue / indigo one. `data-display="screen|tv|projector"` redefines **legibility tokens only** — contrast, font weight, rule thickness, decor strength (`--rule`, `--rule-opacity`, `--weight-body`, `--weight-thin`, `--color-fg`, `--decor-dots`, `--decor-mesh`). A profile never touches geometry, which is what makes it safe to switch mid-talk: it cannot move anything. `projector` sets both decor tokens to 0.
...
| `slide-hero` | marks `cover` and `quote`: the stage reads it to raise both decor layers, and a talk's `em` treatment hangs off it. Carries only `position: relative` itself |
