...
**mermaid keeps its light theme in both modes, on a light ground of its own** (`.dark .mermaid` in `impact-framework/.demoit/style.css`). Its own `dark` theme sizes this deck's `block-beta` nodes narrower than the labels it then paints into them, clipping every one. Tried and did not move it: `base` driven from the deck's tokens, pinning `fontFamily` and `fontSize` (also in `themeVariables`), block padding, and deferring the render until the webfont loaded. **Do not add padding to `.mermaid`** — mermaid measures that container to size its columns, so padding narrows it and clips the labels for real, in both modes. The same trap caught the chassis once: a `display: inline-block` on `svg` compressed that container and clipped the figure everywhere, which is why `styles/demoit.src.css` restores inline images for `img` and `video` only.
